 ...
 * Fecha: 18/03/2023
 * Descripcion: Practica 4, Implementacion del juego de pong en 1D
 *  con el uso de registros
...
#define BTN_BUILTIN_GPIO GPIO_NUM_4
...
void rotateSevenBitsOnce(eDirection_t direction) {
   state = (direction == eGoingLeft) ? ((state << 1) | 1) : ((state >> 1) | (1 << 6));
}
   ...
   switch (gameState) {
      ...
      case eOngoingGame:
         if (!stateFlag) {
            ...
            startTime[0] = _millis;
            ledDelay = 300;
            state = (direction == eGoingRight) ? 1 : 0x40;
         }
      ...
      case eEnd:
         if (!stateFlag) {
            ...
            if (_millis - startTime[0] > LED_OFF_DELAY && state == 0) {
               startTime[0] = _millis;
               state = score;
               // Use the builtin LED to indicate the end of the game
               gpio_set_level(LED_BUILTIN_GPIO, (gpio_get_level(LED_BUILTIN_GPIO) == 0) ? 1 : 0);
...
ePlayerInputState_t checkPlayerInput(eButtonState_t buttonState, eButtonId_t buttonId) {
   if (buttonState == eBtnShortPressed) {
      if (buttonId == eLeftButton)
         return (direction == eGoingLeft && (state == 1 || state == 1<<1)) ? eCorrect : eIncorrect;
      if (buttonId == eRightButton) {
         return (direction == eGoingRight && (state == 1 << 6 || state == 1 << 5)) ? eCorrect : eIncorrect;
      }
   }
   ...
   while (1) {
      ...
      switch (currentGameState) {
         ...
         case eWaitForStart:
            if (buttonState == eBtnShortPressed) {
               _millis = 0;
               direction = (buttonId == eLeftButton) ? eGoingRight : eGoingLeft;
               currentGameState++;
            }
