...
to setup
  ...
  ask patches with [pycor < surface-ycor]
    [ set pcolor gray ]
  set spout-ycor round (max-pycor * 1)
  let spout-space round (max-pxcor * spout-spacing)
...
to spout
  if ticks mod sand-spacing = 0
    [ ask spout-patches with [pxcor < 0 and random-float 100 <= release-chance-left]
      [ ask patch-at 0 -1 [ set pcolor brown ] ]
    ask spout-patches with [pxcor = 0 and random-float 100 <= release-chance-mid]
      [ ask patch-at 0 -1 [ set pcolor brown ] ]
    ask spout-patches with [pxcor > 0 and random-float 100 <= release-chance-right]
      [ ask patch-at 0 -1 [ set pcolor brown ] ]
  ]
end
