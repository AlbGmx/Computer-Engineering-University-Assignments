...
#define BASEANIMATIONSPEED 2000
...
void playAnimation() {
	getAnimationInfo();
	if (animationSpeed == 0) {
		if (milis % 1000 == 0 | playingAnimation) printf("\nAnimation stop at %llums", ledState);
		playingAnimation = false;
	} else {
		if (!playingAnimation) {
			printf("\nPlaying animation from stop / start");
			startTime		  = milis;
			playingAnimation = true;
		} else if (milis - startTime >= animationSpeed) {
			printf("\nPlaying animation at %llums intervals", animationSpeed);
			startTime = milis;
